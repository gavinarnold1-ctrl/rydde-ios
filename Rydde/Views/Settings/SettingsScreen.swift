...
struct SettingsScreen: View {
    ...
    private var soloHouseholdContent: some View {
        ...
        .sheet(isPresented: $viewModel.showShareSheet) {
            if let code = viewModel.inviteCode {
                ShareSheet(items: ["Join my household on Rydde! Use code: \(code)"])
            }
        }
    }
    ...
    private var automationSection: some View {
        SettingsSection(title: "AUTOMATION") {
            ...
            if viewModel.reminderEnabled {
                    ...
                    VStack(alignment: .leading, spacing: RyddeTheme.Spacing.sm) {
                        ...
                        HStack(spacing: RyddeTheme.Spacing.sm) {
                            ForEach([10, 15, 30, 60], id: \.self) { mins in
                                DurationChip(
                                    label: mins >= 60 ? "1 hr" : "\(mins) min",
                                    isSelected: viewModel.reminderDuration == mins
                                ) {
                                    viewModel.reminderDuration = mins
                                    Task { await viewModel.saveAutomation() }
                                }
                            }
                        }
                ...
                .padding(.top, RyddeTheme.Spacing.sm)
                .animation(.easeOut(duration: 0.3), value: viewModel.reminderEnabled)
            }
        }
    }
