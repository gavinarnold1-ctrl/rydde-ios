...
struct EditPainPointsSheet: View {
    ...
    var body: some View {
        NavigationStack {
                ...
                ScrollView {
                    VStack(spacing: RyddeTheme.Spacing.sm) {
                        ForEach(painPoints) { point in
                            HStack {
                                Text(point.description).font(RyddeTheme.Fonts.body).foregroundColor(Color(RyddeTheme.Colors.primaryText))
                                Spacer()
                                Button(action: { painPoints.removeAll { $0.id == point.id }; onDelete(point.id) }) {
                        ...
                        HStack(spacing: RyddeTheme.Spacing.sm) {
                            ...
                            Button(action: { let text = newPainPoint.trimmingCharacters(in: .whitespaces); guard !text.isEmpty else { return }; onAdd([text]); painPoints.append(PainPointItem(id: UUID(), description: text)); newPainPoint = "" }) {
            ...
            .toolbar {
                ToolbarItem(placement: .principal) { Text("Pain points").font(RyddeTheme.Fonts.headingSmall).foregroundColor(Color(RyddeTheme.Colors.primaryText)) }
